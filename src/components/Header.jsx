    ...
    <header className="bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-slate-100 shadow-sm transition-all">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5 flex items-center justify-between">
        ...
        <Link to="/" className="flex items-center gap-2.5 group">
          <img src="/logo.png" alt="Mamun Socks Logo" className="h-auto w-16 md:w-20 rounded-lg" />
          {/* <div>
            <h1 className="text-sm font-extrabold tracking-widest text-slate-950 uppercase">MAMUN SOCKS</h1>
            <div className="flex items-center gap-1 text-[9px] text-slate-600 font-bold tracking-wider">
            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.path}
                  to={link.path}
                  className={`text-xs uppercase tracking-widest font-bold py-1.5 transition-colors ${isActive ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-700 hover:text-slate-950 font-extrabold'}`}
                >
                  {link.label}
                </Link>
              );
          ...
          {user ? (
            <div className="flex items-center gap-3 pl-2 border-l border-slate-100">
              <div className="text-right">
                ...
                <p className="text-[9px] text-slate-605 uppercase tracking-widest font-bold">{user.role}</p>
              </div>
              <button
          ...
          {navLinks.map((link) => {
            ...
            return (
              <Link
                ...
                className={`block px-3 py-2 rounded-xl text-xs font-extrabold uppercase tracking-wider transition-colors ${isActive ? 'bg-emerald-50 text-emerald-700' : 'text-slate-800 hover:bg-slate-50 hover:text-slate-950'}`}
              >
                {link.label}
              </Link>
            );
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-between px-3 text-xs">
            {user ? (
              <>
                ...
                <button
                  onClick={() => { setMobileMenuOpen(false); typeof onLogout === 'function' && onLogout(); }}
